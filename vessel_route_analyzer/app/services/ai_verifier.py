...
class AIVerificationEngine:
    ...
    async def verify_route_threats(self, departure: str, destination: str, eta_scheduled: str, route_points: list) -> dict:
        ...
        executor_result = await run_codex_prompt(executor_prompt, timeout_seconds=240.0)
        ...
        return await run_codex_prompt(reviewer_prompt, timeout_seconds=240.0)
    ...
    def _build_executor_prompt(self, departure: str, destination: str, eta_scheduled: str, coords_text: str) -> str:
        return (
            ...
            f"1. CNN, Reuters, BBC, AP, AFP, Bloomberg, 연합뉴스 등 공신력 있는 주요 매체의 기사만 근거로 사용하십시오.\n"
            f"2. 위 매체 중 하나에서 명확한 기사를 찾으면 충분합니다. 여러 매체를 교차 검증하려 하지 말고 빠르게 판단하십시오.\n"
            f"3. 반드시 검색 도구를 사용해 실제 기사 상세 URL(article_link)을 포함하십시오. 메인 홈페이지, 검색 결과 페이지, 깨진 링크는 금지입니다.\n"
            f"4. 위 매체에서 명확한 이슈를 찾지 못하면 issues를 빈 배열로 반환하십시오.\n"
    ...
    def _build_reviewer_prompt(
        ...
        return (
            ...
            f"1. 실행자 초안의 각 기사 링크가 CNN, Reuters, BBC, AP, AFP, Bloomberg, 연합뉴스 등 공신력 있는 매체의 실제 기사 상세 URL인지만 가볍게 확인하십시오. 과도하게 재검색하지 말고 명백히 문제가 있는 경우만 수정/삭제하십시오.\n"
            f"2. 각 이슈가 항로 좌표와 관련 있는지, 설명과 지연 추정이 크게 과장되지 않았는지 검토하십시오.\n"
            f"3. 필요하면 이슈를 수정, 삭제, 보완하고 총 지연시간과 위험등급을 다시 산정하십시오.\n"
            f"4. source_tier는 다음 기준을 따르십시오: Tier 1=국제 주요 매체, Tier 2=인근국 주요 매체.\n"
            f"5. verification_status는 검수 후 그대로 유지하면 verified, 내용이나 링크를 보정했으면 corrected, 출처가 부적합하면 rejected_reference로 표시하십시오.\n"
            f"6. 검수 결과는 마크다운 없이 반드시 아래 JSON 형식으로만 반환하십시오.\n\n"
            f"최종 반환 JSON 규격:\n"
            f"{{\n"
            f"  \"issues\": [\n"
            f"    {{\n"
